...
// This custom logout is required, as the standard PKCE React library (react-oauth2-pkce)
// doesn't support the id_token_hint mechanism (which is what the OpenText Developer Cloud uses).
// A request with the owners of react-oauth2-pkce has been logged to add id_token_hint support.
// Feel free to inquire on the matter yourself.
const logoutWithIdTokenHint = (shouldEndSession, idToken) => {
  ...
  window.location.replace(
    `${process.env.REACT_APP_BASE_SERVICE_URL}/tenants/${process.env.REACT_APP_TENANT_ID}/oauth2/logout?id_token_hint=${encodeURIComponent(idToken)}&post_logout_redirect_uri=${encodeURIComponent(process.env.REACT_APP_REDIRECT_URI)}`,
  );
};

// If you want to test (access) token expiry, you can enable this interceptor method
// and look at the developer tools console in your browser
// to force a 401/Unauthorized error and see how it gets intercepted
// and remediated (new token fetched) by the reponse interceptor
...
//         "ca_token_expiration_time", Date.now() + timeUntilTokenExpires
//       );
//     } else {
//       if (Date.now() > tokenExpirationTime) {
//         if (config.headers.Authorization !== "Bearer fakebearertoken") {
//           config.headers = {
//             ...config.headers,
