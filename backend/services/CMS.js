...
const cmsGetObject = async (req, category, type, id, authorization) => {
  let getRequest = {
    method: "get",
    url: process.env.BASE_URL + '/cms/instances/' + category + '/' + type + '/' + id + req.originalUrl.replace(/[^?]*/, ""),
    headers: {
      'Authorization': authorization
    }
  };
  ...
  return new Promise((resolve, reject) => {
    request(getRequest, (error, response) => {
      if (error) throw new Error("Error in receiving cms object: " + error);
      if (response.statusCode !== 200) {
        let responseBody = JSON.parse(response.body);
        console.log('Request failed: ', responseBody);
        return reject({
